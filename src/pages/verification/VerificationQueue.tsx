...
function VerificationQueue() {
  ...
  const [priorityFilter] = useState<string>("All Priorities");
  ...
  const queueData: QueueItem[] = localCandidates
    ...
    .map((candidate: Candidate, index: number) => {
      ...
      if (candidate.status === "FRAUD_ALERT") derivedPriority = "Critical";
      else if (candidate.status === "UNDER_VERIFICATION")
        derivedPriority = "High";
      else if (candidate.status === "DOCUMENTS_UPLOADED")
        derivedPriority = "High";
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-gray-500 mt-2">
              Process ongoing modular pipeline applicant validations
            </p>
          </div>
                  ...
                  >
                    ...
                    <td className="p-6 flex justify-center gap-2">
                      <button
                        onClick={() =>
                          navigate(
                            `/verification/candidate/${item.candidateId}`,
                          )
                        }
