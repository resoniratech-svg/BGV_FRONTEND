...
import { Search, CheckCircle, Clock, AlertTriangle, Building2, Users, Briefcase, ShieldCheck } from "lucide-react";
...
function EmploymentVerification() {
  ...
  const verifyEmployment = (candidateId: number) => {
    ...
    alert("Employment verification recorded successfully.");
  };

  const employmentRequests: EmploymentRequest[] = localCandidates
    ...
    .map(c => ({
      ...
      designation: c.role || "Software Engineer",
      duration: "2 Years",
      status: "Pending"
    }));

  const filteredRequests = employmentRequests.filter(item =>
    ...
    <DashboardLayout>
      ...
      <div className="space-y-8 max-w-6xl mx-auto pb-12">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-4xl font-bold text-gray-900">Employment Verification Engine</h1>
            <p className="text-gray-500 mt-2">AI-powered employment history validation and employer authentication</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 flex items-center justify-between">
            <div><p className="text-sm text-gray-500">Total Requests</p><h2 className="text-4xl font-bold mt-2">{employmentRequests.length}</h2></div>
            <div className="p-3 bg-indigo-50 text-indigo-600 rounded-2xl"><Briefcase className="w-5 h-5"/></div>
          </div>
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 flex items-center justify-between">
            <div><p className="text-sm text-gray-500">Verified</p><h2 className="text-4xl font-bold mt-2 text-green-500">{localCandidates.filter(c => c.moduleStatuses?.["Employment"] === "Verified").length}</h2></div>
            <div className="p-3 bg-green-50 text-green-500 rounded-2xl"><CheckCircle className="w-5 h-5"/></div>
          </div>
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 flex items-center justify-between">
            <div><p className="text-sm text-gray-500">Pending</p><h2 className="text-4xl font-bold mt-2 text-yellow-500">{employmentRequests.length}</h2></div>
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-gray-100 flex items-center justify-between gap-4">
            <h2 className="text-xl font-bold">Employment Verification Requests</h2>
            <input
          ...
          <table className="w-full">
            <thead className="bg-[#F8FAFC]">
              <tr>
                ...
                <th className="p-6 text-left text-xs font-semibold text-gray-500 uppercase">Company</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredRequests.map(item => (
                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="p-6 text-sm text-gray-600">{item.company}</td>
                  ...
                  <td className="p-6 text-center">
                    <button onClick={() => verifyEmployment(item.candidateId)} className="bg-emerald-500 hover:bg-emerald-600 text-white px-5 py-2.5 rounded-xl text-xs font-bold transition-all">Verify Employment</button>
                  </td>
                </tr>
              ))}
            </tbody>
