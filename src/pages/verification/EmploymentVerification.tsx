...
function EmploymentVerification() {
  ...
  } = useVerificationModule("employment");
  ...
  const employmentRequests: EmploymentRequest[] = candidates.map((candidate) => ({
    id: `EMP-${candidate.candidate_id}`,
      ...
      <div className="space-y-8 max-w-6xl mx-auto pb-12 mt-4">
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-gray-100 flex items-center justify-between flex-wrap gap-4 bg-white">
            <h2 className="text-xl font-bold text-gray-900">Employment Verification Requests</h2>
            <input
              type="text"
            ...
            <table className="w-full min-w-[850px]">
              <thead className="bg-[#F8FAFC]">
                <tr>
                  ...
                  <th className="p-6 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Company</th>
                ...
                {filteredRequests.length > 0 ? (
                  filteredRequests.map(item => (
                    <tr key={item.id} className="hover:bg-[#FAFBFF] transition-all">
                      ...
                      <td className="p-6 text-sm text-gray-600 font-medium">{item.company}</td>
                      <td className="p-6">
                      ...
                      <td className="p-6 text-center">
                        <button 
                          onClick={() => verifyEmployment(item.candidateId)} 
                  ...
                  <tr>
                    <td colSpan={5} className="p-16 text-center text-gray-400 font-medium text-sm">
                      No active employment verification requests pending.
                    </td>
                  </tr>
                )}
              </tbody>
