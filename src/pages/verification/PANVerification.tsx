...
import { Search, ShieldCheck, CheckCircle, Clock, AlertTriangle, Cpu, CreditCard, Database } from "lucide-react";
...
function PANVerification() {
  ...
  const verifyPAN = (candidateId: number) => {
  ...
  const panRequests: PanRequest[] = localCandidates
    .filter((c: Candidate) => c.status !== "Verified" && !c.panVerified)
    .map((c: Candidate) => ({
      id: `PAN-${1000 + c.id}`,
      candidateId: c.id,
      candidate: c.name || "Unknown",
      pan: c.panNumber || "ABCDE1234F",
      score: "96%",
      status: "Pending",
    }));

  const filteredRequests = panRequests.filter((item) =>
    ...
    <DashboardLayout>
      ...
      <div className="space-y-8 max-w-6xl mx-auto pb-12">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-4xl font-bold text-gray-900">PAN Verification</h1>
            <p className="text-gray-500 mt-2">AI-powered PAN card syntax checking and compliance tracking workflows</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 flex items-center justify-between">
            <div><p className="text-sm text-gray-500">Total Requests</p><h2 className="text-4xl font-bold mt-2">{panRequests.length}</h2></div>
            <div className="p-3 bg-purple-50 text-purple-600 rounded-2xl"><Database className="w-5 h-5"/></div>
          </div>
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 flex items-center justify-between">
            <div><p className="text-sm text-gray-500">Verified</p><h2 className="text-4xl font-bold mt-2 text-green-500">{localCandidates.filter(c => c.panVerified).length}</h2></div>
            <div className="p-3 bg-green-50 text-green-500 rounded-2xl"><CheckCircle className="w-5 h-5"/></div>
          </div>
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 flex items-center justify-between">
            <div><p className="text-sm text-gray-500">Pending</p><h2 className="text-4xl font-bold mt-2 text-yellow-500">{panRequests.length}</h2></div>
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-gray-100 flex items-center justify-between gap-4">
            <h2 className="text-xl font-bold">PAN Verification Requests</h2>
            <input
              ...
              {filteredRequests.map(item => (
                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="p-6 text-center">
                    <button onClick={() => verifyPAN(item.candidateId)} className="bg-emerald-500 hover:bg-emerald-600 text-white px-5 py-2.5 rounded-xl text-xs font-bold transition-all">Verify PAN</button>
                  </td>
                </tr>
              ))}
            </tbody>
