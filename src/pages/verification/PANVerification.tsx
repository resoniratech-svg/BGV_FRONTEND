...
function PANVerification() {
  ...
  const panRequests: PANRequest[] = candidates.map((candidate) => ({
    id: `PAN-${candidate.candidate_id}`,
      ...
      <div className="space-y-8 max-w-6xl mx-auto pb-12 mt-4">
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-gray-100 flex items-center justify-between flex-wrap gap-4 bg-white">
            <h2 className="text-xl font-bold text-gray-900">PAN Verification Requests</h2>
            <input
              type="text"
                ...
                {filteredRequests.length > 0 ? (
                  filteredRequests.map((item) => (
                    <tr key={item.id} className="hover:bg-[#FAFBFF] transition-all">
                      ...
                      <td className="p-6 text-sm text-gray-880 font-semibold">{item.candidate}</td>
                      <td className="p-6">
                      ...
                      <td className="p-6 text-center">
                        <button 
                          onClick={() => verifyPAN(item.candidateId)} 
                  ...
                  <tr>
                    <td colSpan={4} className="p-16 text-center text-gray-400 font-medium text-sm">
                      No active PAN verification requests pending.
                    </td>
                  </tr>
                )}
              </tbody>
