...
import { Search, ShieldCheck, CheckCircle, Clock, AlertTriangle, Cpu, FileText, Globe } from "lucide-react";
...
function PassportVerification() {
  ...
  const verifyPassport = (candidateId: number) => {
    ...
    alert("Passport Verification Completed Successfully.");
  };

  const passportRequests: PassportRequest[] = localCandidates
    .filter((c: Candidate) => c.status !== "Verified" && !c.passportVerified)
    .map((c: Candidate) => ({
      id: `PAS-${1000 + c.id}`,
      candidateId: c.id,
      candidate: c.name || "Unknown",
      passport: c.passportNumber || "P1234567",
      nationality: c.country || "Indian",
      status: "Pending",
    }));
      ...
      <div className="space-y-8 max-w-6xl mx-auto pb-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 flex items-center justify-between">
            <div><p className="text-sm text-gray-500">Total Requests</p><h2 className="text-4xl font-bold mt-2">{passportRequests.length}</h2></div>
            <div className="p-3 bg-purple-50 text-purple-600 rounded-2xl"><FileText className="w-5 h-5"/></div>
          </div>
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 flex items-center justify-between">
            <div><p className="text-sm text-gray-500">Verified</p><h2 className="text-4xl font-bold mt-2 text-green-500">{localCandidates.filter(c => c.passportVerified).length}</h2></div>
            <div className="p-3 bg-green-50 text-green-500 rounded-2xl"><CheckCircle className="w-5 h-5"/></div>
          </div>
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 flex items-center justify-between">
            <div><p className="text-sm text-gray-500">Pending</p><h2 className="text-4xl font-bold mt-2 text-yellow-500">{passportRequests.length}</h2></div>
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-gray-100 flex items-center justify-between gap-4">
            <h2 className="text-xl font-bold">Passport Verification Requests</h2>
            <input
              ...
              {filteredRequests.map(item => (
                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="p-6 text-center">
                    <button onClick={() => verifyPassport(item.candidateId)} className="bg-emerald-500 hover:bg-emerald-600 text-white px-5 py-2.5 rounded-xl text-xs font-bold transition-all">Verify Passport</button>
                  </td>
                </tr>
              ))}
            </tbody>
